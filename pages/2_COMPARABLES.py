...
<div class="fbc-page-header">
    ...
    <span class="fbc-page-header-title">Comparables Valuation</span>
...
def fetch_peer_row_parallel(r, target_profile):
...
def build_live_comps_from_target(target_query: str, max_peers: int = 9, manual_sector_override: str = ""):
    ...
    with ThreadPoolExecutor(max_workers=12) as executor:
        ...
        for i, future in enumerate(as_completed(futures), start=1):
            ...
            status.markdown(f"""
            <div style="font-size:13px; color:#003399;">
            Processing peer {i + 1} of {len(futures)}...
            </div>
            """, unsafe_allow_html=True)
            status.markdown(f"Fetching market data... ({i + 1}/{len(futures)})")
...
if run_live_comps:
    if not target_company.strip():
        ...
        <div class="status">
            🔍 Finding best peers<span class="dots"></span>
        </div>
...
if "comp_exp_0_debug_peer_search" not in st.session_state:
    st.session_state["comp_exp_0_debug_peer_search"] = False
if st.button(
    ("▼ " if st.session_state["comp_exp_0_debug_peer_search"] else "▶ ") + "Debug peer search",
    key="btn_comp_exp_0_debug_peer_search"
):
    st.session_state["comp_exp_0_debug_peer_search"] = not st.session_state["comp_exp_0_debug_peer_search"]
if st.session_state["comp_exp_0_debug_peer_search"]:
...
def fancy_loader(text="Processing...", steps=5):
    progress = progress_bar
    status = search_placeholder

    for i in range(steps):
        pct = int((i + 1) / steps * 100)
        status.markdown(f"**{text} ({pct}%)...**")
        progress.progress(pct)
        time.sleep(0.15)  # small delay for UX feel
